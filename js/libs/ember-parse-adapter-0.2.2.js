...
var ParseJSONSerializer = DS.JSONSerializer.extend({
  ...
  */
  serializeRootObject: function(type, data){
    // flatten pointers to id lists for DS.JSONSerializer
    var relationshipNames = Ember.get(type, 'relationshipsByName').keys.list;
    for ( var i = 0; i < relationshipNames.length; i += 1 ) {
      var relationshipName = relationshipNames[i];
      if ( Ember.isArray(data[relationshipName]) && data[relationshipName][0]['__type'] === "Pointer" ) {
        var ids = [];
        for ( var k = 0; k < data[relationshipName].length; k += 1 ) {
          ids.push(data[relationshipName][k].objectId)
        }
        data[relationshipName] = ids;
      }
    }
